...
export default function NewInternetSpeed() {
  ...
  useEffect(() => {
    if (latestDownloadSpeed) {
      ...
      const sufficientDataPoints =
        newDownloadSpeeds.length >= MAX_REQUESTS_FOR_SPEED_TEST;
  ...
  if (placeFieldsMissing) {
    buttonClass =
      'w-full px-4 py-2 font-bold text-white bg-blue-500 rounded-full focus:outline-none focus:shadow-outline disabled:opacity-25';
  }
